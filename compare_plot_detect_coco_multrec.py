...
from lib.base import (coco2xyxy, init_output_path, get_image_paths, get_img_ids_from_arguments, load_json, get_annoation_data_fpath, 
                        ...
                        rle2mask, get_id_img_paths, load_custom_labels, get_subdir_paths)
...
'''
def plot_compare():
    recordings_path, segmentation, annotator, labels_fpath, weights, save_txt, img_size, image_ids, category_id_is_line, save_results = \
        opt.recordings_path, opt.segmentation, opt.annotator, opt.annotator_labels_path, opt.weights, opt.save_txt, opt.img_size, \
    ...
    for i, recording_path in enumerate(recording_paths):   
        ...
        output_path = os.path.join(coco_path, 'compared_images')
        ...
        for img_id in image_ids:
            ...
            for id_img_yolo_annotation in id_img_yolo_annotations:
                ...
                if category is None:
                    print('Error: category with id {} not found in {}'.format(id_img_yolo_annotation['category_id'], yolo_annotation_data_fpath), file=sys.stderr)
